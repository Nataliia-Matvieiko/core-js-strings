...
function formatTime(minutes, seconds) {
  const resultForMinutes = String(minutes).padStart(2, '0');
  const resultForSeconds = String(seconds).padStart(2, '0');
  return `${resultForMinutes}:${resultForSeconds}`;
}
...
function countVowels(str) {
  const vowels = str
    .toLowerCase()
    .split('')
    .filter((char) => ['a', 'e', 'i', 'o', 'u', 'y'].includes(char));
